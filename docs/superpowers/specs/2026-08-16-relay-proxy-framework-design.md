...
The example proxies handshake, status, and the opening of login with decoding
active, and falls back to opaque passthrough once a session enables encryption.
...
itself and teaches nothing about the framework seam. The example's job is to
...
type Framer interface {
	ReadMessage(*bufio.Reader) ([]byte, error)
